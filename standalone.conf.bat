...
if "x%JBOSS_JAVA_SIZING%" == "x" (
    rem # JVM memory allocation pool parameters - modify as appropriate.
     set "JBOSS_JAVA_SIZING=-Xms1024m -Xmx5096m -XX:MetaspaceSize=256M -XX:MaxMetaspaceSize=1096m"
)
...
set "JAVA_OPTS=%JAVA_OPTS% -Djboss.node.name=node2 -Djboss.bind.address=0.0.0.0 -Djboss.bind.address.private=0.0.0.0 -Djboss.socket.binding.port-offset=100 -Djboss.tx.node.id=node2"

set "JAVA_OPTS=%JAVA_OPTS% -Djboss.messaging.cluster.password=MyStrongClusterPass123!"
...
set "JAVA_OPTS=%JAVA_OPTS% -Duser.language=en -Duser.country=GB"
